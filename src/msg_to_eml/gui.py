...
from . import convert_msg_to_eml, batch_convert
...
class MSGToEMLApp:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="MSG to EML Converter", font=("Arial", 16, "bold")
        )
        ...
        single_frame = ttk.LabelFrame(
            main_frame, text="Convert Single File", padding="10"
        )
        single_frame.grid(
        ...
        ttk.Label(single_frame, text="MSG File:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        self.single_input = ttk.Entry(single_frame, width=50)
        self.single_input.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(0, 10))
        ttk.Button(single_frame, text="Browse", command=self.browse_single_input).grid(
            row=0, column=2
        )

        ttk.Label(single_frame, text="Output EML:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 0)
        )
        self.single_output = ttk.Entry(single_frame, width=50)
        self.single_output.grid(
            row=1, column=1, sticky=(tk.W, tk.E), padx=(0, 10), pady=(10, 0)
        )
        ttk.Button(single_frame, text="Browse", command=self.browse_single_output).grid(
            row=1, column=2, pady=(10, 0)
        )

        ttk.Button(single_frame, text="Convert File", command=self.convert_single).grid(
            row=2, column=1, pady=(15, 0)
        )

        # Batch conversion section
        batch_frame = ttk.LabelFrame(
            main_frame, text="Convert Multiple Files (Batch)", padding="10"
        )
        batch_frame.grid(
        ...
        ttk.Label(batch_frame, text="Input Folder:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        self.batch_input = ttk.Entry(batch_frame, width=50)
        self.batch_input.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(0, 10))
        ttk.Button(batch_frame, text="Browse", command=self.browse_batch_input).grid(
            row=0, column=2
        )

        ttk.Label(batch_frame, text="Output Folder:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 0)
        )
        self.batch_output = ttk.Entry(batch_frame, width=50)
        self.batch_output.grid(
            row=1, column=1, sticky=(tk.W, tk.E), padx=(0, 10), pady=(10, 0)
        )
        self.batch_output_button = ttk.Button(
            batch_frame, text="Browse", command=self.browse_batch_output
        )
        ...
        output_location_check.grid(
            row=3, column=0, columnspan=2, sticky=tk.W, pady=(5, 0)
        )

        ttk.Button(
            batch_frame, text="Convert All Files", command=self.convert_batch
        ).grid(row=4, column=1, pady=(15, 0))

        # Progress bar
        self.progress = ttk.Progressbar(main_frame, mode="indeterminate")
        self.progress.grid(
            row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(20, 10)
        )
        ...
        self.log_message(
            "Choose single file conversion or batch conversion for multiple files."
        )
        self.log_message(
            "📁 Batch mode supports recursive directory scanning (searches subfolders)."
        )
        self.log_message(
            "📍 Option: Create EML files next to original MSG files or in separate folder."
        )
    ...
    def toggle_output_location(self):
        ...
        if self.output_next_to_original_var.get():
            ...
            self.log_message(
                "📍 Mode: EML files will be created next to original MSG files"
            )
        else:
            ...
            self.log_message(
                "📁 Mode: EML files will be created in separate output folder"
            )
    ...
    def convert_batch(self):
        ...
        if not output_next_to_original:
            ...
            if not output_dir:
                messagebox.showerror(
                    "Error",
                    "Please select an output folder or check 'Create EML files next to original MSG files'.",
                )
                return
        else:
        ...
        threading.Thread(
            ...
            args=(input_dir, output_dir, output_next_to_original),
            daemon=True,
        ).start()
    ...
    def _convert_batch_thread(self, input_dir, output_dir, output_next_to_original):
        ...
        try:
            ...
            stats = batch_convert(
                ...
                output_dir,
                recursive=recursive,
                output_next_to_original=output_next_to_original,
            )
            ...
            if stats["msg_files_found"] == 0:
                ...
                if stats["files_ignored"] > 0:
                    self.log_message(
                        f"⚠️  {stats['files_ignored']} non-MSG files ignored"
                    )
                messagebox.showwarning(
                    "No MSG Files", "No MSG files found in the selected folder."
                )
                return
            ...
            if stats["files_ignored"] > 0:
                ...
                if stats["files_ignored"] <= 5 and "ignored_files" in stats:
                    ...
                    self.log_message(
                        f"   (Too many to list - includes documents, images, etc.)"
                    )

            # Show conversion results
            if stats["files_converted"] > 0:
                self.log_message(
                    f"✅ Successfully converted {stats['files_converted']} files"
                )
