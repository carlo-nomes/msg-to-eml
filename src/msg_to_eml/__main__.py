...
Examples:
  ...
  python -m msg_to_eml input_folder/ output_folder/ --batch --no-recursive
    ...
    parser.add_argument("output", nargs="?", help="Output EML file or directory for EML files (optional when using --next-to-original)")
    parser.add_argument("--batch", action="store_true", help="Batch convert all MSG files in input directory")
    parser.add_argument("--recursive", action="store_true", default=True, help="Search subdirectories recursively (default: True)")
    parser.add_argument("--no-recursive", dest="recursive", action="store_false", help="Do not search subdirectories recursively")
    parser.add_argument("--next-to-original", action="store_true", help="Create EML files next to original MSG files (ignores output path)")
    ...
    if not args.next_to_original and not args.output:
        if is_batch:
            print("Error: Output directory is required for batch conversion unless using --next-to-original", file=sys.stderr)
        else:
            print("Error: Output file path is required for single file conversion", file=sys.stderr)
        return 1
    ...
    try:
        if is_batch:
            ...
            assert output_path is not None, "Output path is required for single file conversion"
